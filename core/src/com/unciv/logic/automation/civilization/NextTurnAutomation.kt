...
object NextTurnAutomation {
    ...
    fun getUnitPriority(unit: MapUnit, isAtWar: Boolean): Int {
        ...
        return distance - unit.getTile().getTilesInDistance(1).count { it.militaryUnit?.civ == unit.civ } * 2 + (unit.health / 10) + when {
            unit.hasUnique(UniqueType.CanMoveAfterAttacking) -> 9
    ...
    private fun trainSettler(civInfo: Civilization) {
        ...
        if (civInfo.gameInfo.tileMap.values.count { it.isLand } / civInfo.gameInfo.civilizations.count { it.isMajorCiv() && it.isAlive() } <= civInfo.cities.size * 15) return // 15 land tiles per city?
        ...
        if (civInfo.units.getCivUnits().count { it.hasUnique(UniqueType.FoundCity, StateForConditionals.IgnoreConditionals) } > 1) return
